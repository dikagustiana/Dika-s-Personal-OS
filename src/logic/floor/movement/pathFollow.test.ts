...
describe('path following', () => {
  ...
  it('a trip to a workstation ends at its stand anchor, not the tile centre, via the approach tile', () => {
    ...
    const desk = furnitureOfType(CAMPUS, 'workstation', 'bay-framing-office')[0];
    const from = hexToWorld(start);
    const plan = planTrip(CAMPUS, graph, from, start, desk.hex, 'anchor_sit');
  ...
  it('the client walk time agrees with the server schedule for the same route', () => {
    const start = freeTilesOfZone(CAMPUS, 'program-office')[0];
    const desk = furnitureOfType(CAMPUS, 'workstation', 'bay-domain-synthesis')[2];
    const plan = planTrip(CAMPUS, graph, hexToWorld(start), start, desk.hex, 'anchor_sit');
  ...
  it('an unreachable destination is flagged, not silently teleported', () => {
    ...
    const plan = planTrip(CAMPUS, graph, hexToWorld(start), start, sand.hex, 'anchor_stand');
