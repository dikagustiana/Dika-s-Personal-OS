...
import { TIME_ZONE_CHOICES, formatHour, systemTimeZone } from '../../../../logic/floor/time/clock';
...
export function DevPanel() {
  ...
  return (
    <aside className="floor-panel absolute bottom-4 left-4 w-[340px] p-3 text-sm" aria-label="Developer controls">
      <div className="mb-2 flex items-center justify-between">
        <span className="font-semibold">Time of day</span>
        <span className="tabular-nums text-xs text-foreground-muted">{formatHour(hour)}</span>
      </div>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        <div className="floor-seg" role="group" aria-label="Clock source">
        ...
        <button
          type="button"
          className="rounded-md border border-border px-2.5 py-1 text-xs text-foreground hover:border-primary"
      ...
      <label className="mt-2 block text-xs text-foreground-muted">
        Time zone
        <select
          className="mt-1 w-full rounded-md border border-border bg-transparent px-2 py-1 text-sm text-foreground"
          value={s.timeZone}
          onChange={(e) => s.setTimeZone(e.target.value)}
        >
          {zones.map((z) => (
            <option key={z} value={z} className="bg-surface-1">
              {z}
            </option>
          ))}
        </select>
      ...
      <div className="mt-3 border-t border-border pt-3">
        <div className="mb-1 text-xs text-foreground-muted">Hex grid</div>
        <div className="floor-seg" role="group" aria-label="Hex grid visibility">
          {GRID_MODES.map((m) => (
      ...
      <label className="floor-toggle mt-2">
      ...
      <dl className="mt-3 grid grid-cols-4 gap-2 border-t border-border pt-3 tabular-nums text-xs">
        <div>
          <dt className="text-foreground-muted">fps</dt>
          <dd className={s.perf.fps >= 55 ? 'text-success' : s.perf.fps >= 30 ? 'text-escalate' : 'text-destructive'}>{s.perf.fps}</dd>
        </div>
        <div>
          <dt className="text-foreground-muted">ms</dt>
          <dd>{s.perf.frameMs}</dd>
        </div>
        <div>
          <dt className="text-foreground-muted">draws</dt>
          <dd>{s.perf.drawCalls}</dd>
        </div>
        <div>
          <dt className="text-foreground-muted">tris</dt>
