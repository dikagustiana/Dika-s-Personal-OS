...
import { HEX_RADIUS, hexToWorld, parseHexKey, sharedEdgeCorners, type WorldXZ } from '../../../../../logic/floor/hex/hex';
...
function edgeSegments(): EdgeSeg[] {
  ...
  for (const z of activeCampus().zones) for (const [a, b] of z.doors) doorKeys.add([a, b].map((h) => `${h.q},${h.r}`).sort().join('|'));
  const out: EdgeSeg[] = [];
  const keys = new Set<string>([...activeCampus().walls, ...doorKeys]);
  for (const key of keys) {
    ...
    if (!seg) continue;
    const ta = activeCampus().tiles.get(ka);
...
export function Architecture() {
  ...
  const interiorTiles = useMemo(() => {
    const walled = new Set(activeCampus().zones.filter((z) => z.walled).map((z) => z.id));
    return Array.from(activeCampus().tiles.values()).filter((t) => t.zoneId && walled.has(t.zoneId));
  }, []);
