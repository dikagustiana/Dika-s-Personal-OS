...
import { useUiStore, type PerfSample } from '../store/uiStore';

declare global {
  interface Window {
    __floorPerf?: PerfSample & { samples: number[] };
  }
}
...
export function PerfMeter() {
  ...
  useFrame((_, delta) => {
    ...
    if (acc.current >= 1) {
      ...
      if (typeof window !== 'undefined') window.__floorPerf = { ...sample, samples: [...history.current] };
      frames.current = 0;
      acc.current = 0;
    }
    gl.info.reset();
