...
import { hexDistance, hexEquals, hexKey } from '../../../../../logic/floor/hex/hex';
import type { CanonicalEvent } from '../../../../../logic/floor/events/schema';
import { CAMPUS, freeTilesOfZone, furnitureOfType } from '../../../../../logic/floor/layout/campus';
import { anchorWorldPose } from '../../../../../logic/floor/layout/resolve';
import { WALK_SPEED_MPS } from '../../../../../logic/floor/movement/movement';
import { pathLength } from '../../../../../logic/floor/movement/pathFollow';
...
// Zones of the INSTITUTION floorplan. The standalone build's 'lounge',
// 'engineering' and 'meeting-a' do not exist here: every department has its
// own bay, lead office and review cluster, and there is no lounge at all.
const open = freeTilesOfZone(CAMPUS, 'program-office');
const desk = furnitureOfType(CAMPUS, 'workstation', 'bay-verification')[0];
const chair = furnitureOfType(CAMPUS, 'meetingChair', 'meet-verification')[1];
...
function ev(over: Partial<CanonicalEvent>): CanonicalEvent {
  ...
  return {
    ...
    currentLocationHex: open[0],
    targetDestinationHex: null,
    currentTaskId: 'task-1',
    collaborationGroupId: null,
    taskDetails: { title: 'T', department: 'verification', progressPercentage: 10, activeSubtask: 'S', tokensUsed: 1 },
    ...over,
  };
}
...
function fresh(id = 'a') {
  removeRuntime(id);
  return getOrCreateRuntime(id, open[0]);
}
...
describe('reconcile: stream → motion', () => {
  ...
  it('WALKING starts a walk toward the target and, on arrival without a new event, stands and waits', () => {
    const rt = fresh();
    reconcile(rt, ev({ currentState: 'OFFICE_IDLE', currentLocationHex: open[0] }), true);
    reconcile(rt, ev({ currentState: 'OFFICE_WALKING', currentLocationHex: open[0], targetDestinationHex: desk.hex }), false);
  ...
  it('a WALKING update reporting a position far ahead of a lagging avatar moves it there and re-plans', () => {
    ...
    const near = freeTilesOfZone(CAMPUS, 'bay-verification')[0];
  ...
  it('a location far from the avatar snaps; a near one catches up with a short walk', () => {
    const rt = fresh();
    reconcile(rt, ev({ currentState: 'OFFICE_IDLE', currentLocationHex: open[0] }), true);
    // Far: the desk is more than CATCHUP_MAX_TILES from the program office.
    reconcile(rt, ev({ currentState: 'OFFICE_WORKING', currentLocationHex: desk.hex }), false);
    expect(rt.follow).toBeNull();
    expect(rt.pose).toBe('sit');
    // Near: two tiles away inside the bay → a brisk walk, not a snap.
    const near = freeTilesOfZone(CAMPUS, 'bay-verification').find((h) => {
  ...
  it('ERROR at the desk keeps the agent seated but stops the typing and raises the alert', () => {
    const rt = fresh();
    reconcile(rt, ev({ currentState: 'OFFICE_WORKING', currentLocationHex: desk.hex }), true);
    reconcile(rt, ev({ currentState: 'OFFICE_ERROR', currentLocationHex: desk.hex, taskDetails: { title: 'T', department: 'verification', progressPercentage: 40, activeSubtask: 'Tests failed', tokensUsed: 5 } }), false);
    expect(rt.pose).toBe('sit');
    expect(rt.activity).toBe('idle');
  ...
  it('WORKING on a tile without a desk stands the agent there with a visible NO DESK alert', () => {
    const rt = fresh();
    reconcile(rt, ev({ currentState: 'OFFICE_WORKING', currentLocationHex: open[2] }), true);
    expect(rt.pose).toBe('stand');
