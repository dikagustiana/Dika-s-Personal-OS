...
export class Positioner {
  ...
  apply(ev: SemanticEvent, nowMs: number): Emission[] {
    ...
    switch (ev.state) {
      ...
      case 'idle': {
        ...
        const spot = this.seating.loungeSpotFor(ev.agentId) ?? here;
        return this.travel(a, ev, nowMs, here, spot, 'OFFICE_WALKING', 'OFFICE_IDLE', null, null);
      }
      case 'working': {
        const desk = this.seating.homeDeskFor(ev.agentId, ev.department);
      ...
      case 'collaborating': {
        ...
        const chair = this.seating.meetingChairFor(groupId, ev.agentId);
  ...
  private ensure(ev: SemanticEvent, nowMs: number): AgentPhysical {
    ...
    if (!a) {
      const spot = this.seating.loungeSpotFor(ev.agentId) ?? { q: 0, r: 0 };
      a = { location: spot, walk: null, activeGroup: null };
