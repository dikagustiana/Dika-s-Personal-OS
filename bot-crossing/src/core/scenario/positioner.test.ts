...
describe('positioner', () => {
  it('emits WALKING now and WORKING at the desk after a real walk, all valid on the wire', () => {
    ...
    expect(arrive.atMs).toBeGreaterThan(walk.atMs + 3000);
    ...
    // The desk is a workstation in the Engineering bay.
    const desk = CAMPUS.furnitureByHex.get(hexKey(arrive.event.currentLocationHex));
    expect(desk?.type).toBe('workstation');
    expect(desk?.zoneId).toBe('engineering');
  });

  it('starts every agent in the lounge and gives each its own spot', () => {
    const p = new Positioner(CAMPUS);
    const spots = new Set<string>();
    for (let i = 0; i < 12; i++) {
      const out = p.apply({ ...base, agentId: `agent-${i}`, state: 'idle' }, 0);
      expect(out).toHaveLength(1);
      expect(out[0].event.currentState).toBe('OFFICE_IDLE');
      expect(CAMPUS.tiles.get(hexKey(out[0].event.currentLocationHex))?.zoneId).toBe('lounge');
      spots.add(hexKey(out[0].event.currentLocationHex));
    }
  ...
  it('three agents sharing a collaborationGroupId get distinct chairs around one table', () => {
    ...
    for (const id of ids) {
      ...
      expect(chair!.zoneId).toBe('meeting-a');
    }
    expect(new Set(seats.values()).size).toBe(3);
    // A second group takes the other room.
    const other = p.apply({ ...base, agentId: 'z', state: 'collaborating', groupId: 'g2' }, 0);
    const chair = CAMPUS.furnitureByHex.get(hexKey(other[other.length - 1].event.currentLocationHex));
    expect(chair?.zoneId).toBe('meeting-b');
  });
  ...
  it('delivering moves as DELIVERING with the terminal as target and arrives with target null', () => {
    ...
    const desk = furnitureOfType(CAMPUS, 'managerDesk')[0];
