'use client';
import type { TokenSample } from '@/core/events/reducer';
...
export function TokenGraph({ samples, width = 300, height = 44 }: { samples: TokenSample[]; width?: number; height?: number }) {
  if (samples.length < 2) {
    return (
      <div className="flex h-[44px] items-center text-2xs text-ink-muted" role="img" aria-label="Token usage: not enough samples yet">
