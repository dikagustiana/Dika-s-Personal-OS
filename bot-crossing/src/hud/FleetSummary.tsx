...
export function FleetSummary() {
  ...
  if (records.length === 0) return <span className="text-xs text-ink-muted">Waiting for agents</span>;
  ...
  const parts = ORDER.filter(([k]) => counts.get(k)).map(([k, label]) => (
    <span key={k} className={k === 'OFFICE_ERROR' ? 'text-error' : k === 'OFFICE_COLLABORATING' ? 'text-accent' : 'text-ink-muted'}>
      {counts.get(k)} {label}
    </span>
  ));
  return (
    <span className="flex flex-wrap items-baseline gap-x-2 text-xs" aria-label="Fleet summary">
      <span className="text-ink">{records.length} agents</span>
      {parts}
      {unknown ? <span className="text-error">{unknown} unknown state</span> : null}
    </span>
  );
}
